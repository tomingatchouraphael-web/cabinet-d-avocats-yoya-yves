...
export const BottomNav: React.FC<BottomNavProps> = ({ currentScreen, onNavigate }) => {
  return (
    <nav
      className="fixed bottom-0 w-full z-50 pb-safe bg-[#f7f9fb]/95 backdrop-blur-xl shadow-[0_-1px_12px_rgba(0,0,0,0.06)] border-t border-[#e2e8f0]/80"
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center flex-1 h-full py-1 transition-all group cursor-pointer relative ${
                isActive ? 'text-[#725b38] font-semibold' : 'text-[#45464d] hover:text-[#0B1120]'
              }`}
              id={`nav-item-${item.id}`}
            >
              {isActive && (
                <span className="absolute top-0 w-8 h-0.5 bg-[#725b38] rounded-full" />
              )}
              <span
